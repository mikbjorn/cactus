...
preds.xgb<- predict(md.xgb, valid_x) 
...
## AVERAGING
est<- data.frame(k=pred.prob, x=preds.xgb)%>% as.matrix()
numparts<- ncol(est)
sumparts<- 500
...
for (i in 1:ncol(weights)){
  ...
  acc<- c(acc, mean(as.numeric(preds>0.6)== valid_y))
}

w<- weights[,which.max(acc)]

est.test<- data.frame(k=model %>% predict(test_x_reshape), 
                      x= preds.xgb<- predict(md.xgb, test_x))%>% as.matrix()

preds<-as.numeric(rowSums(t(t(est.test)*w))>0.6)
